...
b64 = base64.b64encode("{client_id}:{client_secret}".format(client_id = data['client_id'], client_secret = data['client_secret']).encode())
header = {
...
header = {'User-Agent':'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/58.0.3029.96 Safari/537.36',
        'Authorization':authval,
        'Content-Type':'application/x-www-form-urlencoded'
        }
data = {
        "devicePid": ["XSCY10180101TEST0100029"],
        "collectCode": [ "C1", "C2", "C3", "C4", "C5", "C6", "C7", "C8", "C9", "C10", "C11", "C12", "C13", "C14", "C15", "C16", "C17", "C18"]
    }
device = requests.post(url = deviceapi, data = data, headers = header).json()
print(device)
