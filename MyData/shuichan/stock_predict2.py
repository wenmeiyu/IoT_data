...
def train_lstm(batch_size,time_step,train_begin,train_end):
    ...
    batch_index,train_x,train_y=get_train_data(60,20,0,5800)
    pred,_=lstm(X)
...
def prediction(time_step):
    X=tf.placeholder(tf.float32, shape=[None,time_step,input_size])
    #Y=tf.placeholder(tf.float32, shape=[None,time_step,output_size])
    mean,std,test_x,test_y=get_test_data(time_step,5800)
    pred,_=lstm(X)
    # pred, final_states = lstm(X)
