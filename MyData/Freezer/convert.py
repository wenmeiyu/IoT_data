...
# 读入
data = pd.read_csv('sheb3.csv',parse_dates=['time'],  encoding='utf-8')
...
for timelist in all_time_list:
    ...
    # to_add = [ct, real, expected, cp]
    results.loc[len(results)] = to_add


# print(results.head())
results.to_csv('outputsheb3.csv', index=False,  encoding='gbk')
