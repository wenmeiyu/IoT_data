...
# ————————————————预测模型————————————————————
def prediction():
    pred, final_states = lstm(1)  # 预测时只输入[1,time_step,input_size]的测试数据
    saver = tf.train.Saver(tf.global_variables())
    with tf.Session() as sess:
        # 参数恢复
        ...
        pre.to_csv('../Data/stock_data_predict.csv')
        # 以折线图表示结果
        plt.figure()
...
if os.access("./model/stock2.model.meta", os.F_OK):
