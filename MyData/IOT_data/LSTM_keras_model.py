...
def load_data(filename, seq_len, normalise_window):
    ...
    adata = pd.read_csv(filename, encoding='utf-8', parse_dates=['timestamp'], index_col='timestamp',
                        date_parser=dateparse)
    ts = np.array(adata['temperature'])  # 获取温度序列
    t_data = np.array(ts)  # np.ndarray()
    data = t_data.tolist()
...
if __name__ == '__main__':
    ...
    X_train, y_train, X_test, y_test = load_data('../Data/dataA1_0515-19.csv', seq_len, True)
    ...
    model = build_model([1, 70, 85, 1])
    model.fit(X_train, y_train, batch_size=512, nb_epoch=epochs, validation_split=0.05)
    point_by_point_predictions = predict_point_by_point(model, X_test)
    pre = pd.DataFrame(point_by_point_predictions)
    pre.to_csv('point_by_point_predictions.csv')
    print('point_by_point_predictions type:', type(point_by_point_predictions))
